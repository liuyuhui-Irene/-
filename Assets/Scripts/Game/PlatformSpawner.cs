...
public class PlatformSpawner : MonoBehaviour
{
    ...
    private void SpawnPlatform()
    {
        ...
        else if (spawnPlatformCount == 0)
        {
            ...
            //Generando una plataforma de combinación universal生成通用组合平台
            if (ran == 0)
            {
                SpawnCommonPlatformGroup(ranObstacleDir);
            }
            //Generar plataforma de combinación de temas生成主题组合平台
            else if (ran == 1)
            {
                switch (groupType)
                {
            ...
            //Generar plataforma de combinación de obstáculos生成钉子组合平台
            else
            {
                int value = -1;
                if (isLeftSpawn)
                {
                    value = 0;//Genera obstáculos a la derecha生成右边方向得钉子
                }
                else
                {
                    value = 1;//Genera obstáculos a la izquierda生成左边方向得钉子
                }
    ...
    private void AfterSpawnSpike()
    {
        if (afterSpawnSpikeSpawnCount > 0)
        {
            ...
            for (int i = 0; i < 2; i++)
            {
                ...
                if (i == 0)//Genera la plataforma de la dirección original.生成原来方向的平台
                {
                    temp.transform.position = platformSpawnPosition;
                    //如果钉子在左边，原先路径就是右边
                    //Si el clavo está a la izquierda, la ruta original está a la derecha.
                    if (spikeSpawnLeft)
                    {
                        platformSpawnPosition = new Vector3(platformSpawnPosition.x + vars.nextXPos,
                            platformSpawnPosition.y + vars.nextYPos, 0);
                ...
                else//Plataforma para generar dirección de clavos生成钉子方向的平台
                {
